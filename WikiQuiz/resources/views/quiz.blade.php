...
<body>
    ...
    <nav class="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        ...
        <p class="text-md md:block font-medium hidden text-gray-700">Psilocybin mushroom - Quiz</p>

        <button class="text-white bg-slate-700 hover:bg-slate-800 text-sm font-medium px-4 py-2 rounded-lg">
            Exit Quiz
        </button>
    </nav>

    <p class="text-md w-full text-center mt-5 md:hidden font-medium block text-gray-700">Psilocybin mushroom - Quiz</p>
    ...
    <div class="max-w-2xl mx-auto px-4 mt-8">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-8">
            ...
            {{-- Question --}}
            <p class="text-sm text-gray-700 leading-relaxed mb-8" id="question-text">
                High speed storage elements used in the CPU for temporary storage of frequently used operands.
            </p>
            ...
            <div class="flex flex-col gap-3" id="choices">
                <button onclick="selectAnswer(this)" class="w-full text-left px-4 py-3 text-sm text-gray-700 border border-gray-200 rounded-xl hover:border-slate-400 hover:bg-slate-50 transition-all">
                    Processors
                </button>
                <button onclick="selectAnswer(this)" class="w-full text-left px-4 py-3 text-sm text-gray-700 border border-gray-200 rounded-xl hover:border-slate-400 hover:bg-slate-50 transition-all">
                    Registers
            ...
            <div class="mt-8 flex justify-end">
                <button onclick="nextQuestion()" class="bg-slate-700 hover:bg-slate-800 text-white text-sm font-medium px-6 py-2.5 rounded-lg transition-all">
                    Next →
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function selectAnswer(el) {
            document.querySelectorAll('#choices button').forEach(btn => {
                btn.classList.remove('border-slate-600', 'bg-slate-50', 'text-slate-700', 'font-medium');
                btn.classList.add('border-gray-200', 'text-gray-700');
            });

            el.classList.add('border-slate-600', 'bg-slate-50', 'text-slate-700', 'font-medium');
